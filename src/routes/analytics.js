...
// GET /analytics/count — countDocuments (agregación simple)
// Rubrica: Agregaciones simples 5pts
...
// GET /analytics/distinct — distinct values (agregación simple)
// Rubrica: Agregaciones simples — distinct
...
// GET /analytics/top-restaurants — Pipeline 1: best rated ($group + $match + $lookup)
// Rubrica: Agregaciones complejas Pipeline 1
...
// GET /analytics/best-selling-items — Pipeline 2: top items ($unwind + $group)
// Rubrica: Agregaciones complejas P2 + Arrays ($unwind sobre array embebido)
router.get("/best-selling-items", async (req, res) => {
  try {
    const db = getDb();
...
// GET /analytics/revenue-by-month — Pipeline 3: monthly sales ($group temporal)
// Rubrica: Agregaciones complejas P3
...
// GET /analytics/rating-distribution/:restaurantId — Pipeline 4: $group + $push
// Rubrica: Agregaciones complejas P4
router.get("/rating-distribution/:restaurantId", async (req, res) => {
  try {
    const db = getDb();
      ...
      .aggregate([
        ...
        { $sort: { _id: 1 } },
        {
          $group: {
            ...
            total: { $sum: "$count" },
            avgRating: { $avg: { $multiply: ["$_id", "$count"] } },
          },
        },
...
// GET /analytics/order-velocity/:restaurantId — Pipeline 5: $dateTrunc 5min windows (Time Series)
// Rubrica: Agregaciones complejas P5 + Time Series
router.get("/order-velocity/:restaurantId", async (req, res) => {
  try {
    const db = getDb();
    ...
    const results = await db
      ...
      .aggregate([
        {
          $match: {
            "metadata.restaurantId": new ObjectId(req.params.restaurantId),
            toStatus: "pending",
            timestamp: { $gte: since },
          },
        },
...
// GET /analytics/avg-transition-time/:restaurantId — Pipeline 6: avg duration by FSM transition
router.get("/avg-transition-time/:restaurantId", async (req, res) => {
  try {
    const db = getDb();
    ...
    const results = await db
      ...
      .aggregate([
        ...
        {
          $group: {
            _id: { from: "$fromStatus", to: "$toStatus" },
            avgDurationSec: { $avg: "$durationFromPrevSec" },
            count: { $sum: 1 },
          },
        },
...
// GET /analytics/tags — Tags más usados en reseñas ($unwind array)
// Rubrica: Manejo de Arrays en agregaciones ($unwind)
...
// GET /analytics/allergens — Alérgenos más comunes ($unwind array)
// Rubrica: Manejo de Arrays en agregaciones ($unwind)
...
// GET /analytics/revenue-by-category — Revenue por categoría (embebido $unwind + $lookup)
// Rubrica: Manejo de documentos embebidos en agregaciones
...
// GET /analytics/daily-revenue — Query daily_revenue OLAP collection
...
// GET /analytics/restaurant-stats — Query restaurant_stats OLAP collection
...
// POST /analytics/run-batch — Trigger batch jobs manually (for demo)
// Rubrica: Materialized view $merge
router.post("/run-batch", async (req, res) => {
  try {
    ...
    res.status(400).json({ error: "job must be 'daily' or 'weekly'" });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
